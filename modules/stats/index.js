...
const {filterNode,isHidden}=require("../../lib/hide-stats");
...
async function getStats(isAdmin=false){
    ...
    const bySid=new Map(servers.map(s=>[s.sid,s]));
    for(const {sid,status} of servers)if(status==1||(status==2&&isAdmin)){
        if(!stats[sid])continue;
        const server=bySid.get(sid);
        const entry=filterNode(stats[sid],server?.data?.hide_stats,isAdmin);
        if(!isAdmin&&server?.data?.hide_stats?.length)entry.hidden=server.data.hide_stats;
...
svr.get("/stats/:sid",async(req,res)=>{
    ...
    res.render("stat",{
        sid,node,
        hide_stats:req.admin?[]:hideStats,
        traffic:hideCharts?null:await db.traffic.get(sid),
...
svr.get("/stats/:sid/data",async(req,res)=>{
    ...
    const server=await db.servers.get(sid);
    const node=filterNode(stats[sid],server?.data?.hide_stats,req.admin);
    res.json({sid,...node});
});
